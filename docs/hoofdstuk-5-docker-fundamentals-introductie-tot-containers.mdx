...
**Container** — een **draaiend exemplaar** van een image. Je kunt van dezelfde image meerdere containers starten (bv. twee webserver-containers op verschillende hostpoorten).

**Tag** — een versielabel bij een image, bijvoorbeeld `httpd:2.4` of `postgres:16`. `latest` is geen garantie voor “nieuwste stabiel”; in productie kies je liever een **vaste tag**.
...
### Wat heb je nodig om Docker te draaien?
...
Container-filesystem is **tijdelijk**: na verwijderen is data weg. Voor databases of uploads gebruik je een **volume**:
...
Korte voorbeelden (image naar keuze):
...
Dit sluit aan bij *OLR 07*: na deployment (of lokaal draaien) **verifieer** je bereikbaarheid en gebruik **logs** om problemen te lokaliseren.
...
- Met **`docker pull` / `docker run` / `docker ps` / `docker logs` / volumes en `-p`** deploy je stack-componenten naar keuze uit Docker Hub.
...
- **Beschrijving:** Start, stopt of herstart een bestaande container.
- **Voorbeelden:**
...
- **Beschrijving:** Maakt een nieuw image van een container (*niet de aanbevolen workflow* — gebruik liever een Dockerfile, zie hst. 6).
